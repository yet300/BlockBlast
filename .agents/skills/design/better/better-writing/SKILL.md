...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) is covered by the `design-better-typography` skill; error markup and announcements (`aria-invalid`, live regions) by the `design-better-accessibility` skill; room for translated strings by the `design-better-layout` skill.
...
Use this format only when the user asks for a standalone writing review. When `design-better-interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
