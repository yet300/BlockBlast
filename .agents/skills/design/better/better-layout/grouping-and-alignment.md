...
- Numbers in tables right-align to the trailing edge (see `design-better-typography` for tabular figures); text left-aligns to the leading edge.
...
- One primary action per view (see `design-better-colors` for how color enforces this).
