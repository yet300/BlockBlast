...
Hit-area sizes and focus behavior are covered by the `design-better-accessibility` skill; visual polish (radius, shadows, animation) by the `design-better-ui` skill; line length and text spacing by the `design-better-typography` skill.
...
Without an established density system, start with `12px` between adjacent bordered or filled controls and `24px` of clearance around borderless text- and icon-only controls. Compact layouts may use less when `design-better-accessibility` hit areas do not overlap and the controls remain visually distinct.
...
Use this format only when the user asks for a standalone layout review. When `design-better-interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
