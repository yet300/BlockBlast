...
description: >-
  User-invoked, cross-discipline interface review that coordinates design-better-accessibility, design-better-layout, design-better-writing, design-better-typography, design-better-colors, and design-better-ui. Use when explicitly invoked for a holistic review of a screen, flow, feature, or product interface. Supports quick and full review modes. Triggers on design-better-interface, full interface review, holistic UI audit, cross-discipline design review, review the whole interface.
---
...
This skill owns orchestration only. Accessibility rules belong to `design-better-accessibility`; structure to `design-better-layout`; copy to `design-better-writing`; type to `design-better-typography`; color to `design-better-colors`; visual polish and motion to `design-better-ui`. Never duplicate or override their rules here.
...
Identify the framework, styling system, component library, design tokens, supported viewports, and available preview or test commands. Load the foundational application design system ([`design-app-system`](../../app-system/SKILL.md)) to anchor the review in the project's specific brand identity (colors, typography, component specs). Follow the project's established Tailwind, plain CSS, CSS-in-JS, token, and component conventions.
...
1. `design-better-accessibility`
2. `design-better-layout`
3. `design-better-writing`
4. `design-better-typography`
5. `design-better-colors`
6. `design-better-ui`

This skill owns the final response. When a domain skill is loaded through `design-better-interface`, apply its principles and references but ignore its standalone **Review Output Format**. Use the consolidated format, shared severity, and finding cap in this file instead.
