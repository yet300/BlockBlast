...
Typography (text wrapping, font rendering, tabular numbers, spacing) is covered by the `design-better-typography` skill; use that for anything text-related. Accessibility (hit areas, focus states, keyboard support, ARIA, reduced motion) is covered by the `design-better-accessibility` skill. Layout structure (grouping, spacing between sections, breakpoints, spatial RTL) is covered by the `design-better-layout` skill.
...
Use this format only when the user asks for a standalone UI-polish review. When `design-better-interface` orchestrates the review, provide domain evidence and findings to that skill and let its output format, severity scale, consolidation rules, cap, and verdict take precedence.
